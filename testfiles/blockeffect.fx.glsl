...
void main() {
    ...
    xl_retval = SimpleMainVS( vec4(a_position), vec3(a_normal), vec4(a_color), vec4(a_color2), vec2(a_texCoord));
...
void main() {
    SimpleVSOut xl_retval;
    xl_retval = TransparentSimpleMainVS( vec4(a_position), vec3(a_normal), vec4(a_color), vec4(a_color2), vec2(a_texCoord));
    gl_Position = vec4(xl_retval.pos);
    xlv_TEXCOORD0 = vec2(xl_retval.texcoord);
    xlv_COLOR0 = vec4(xl_retval.color);
}
